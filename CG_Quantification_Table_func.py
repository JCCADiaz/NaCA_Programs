...
def top_CG_Gas_Qantification_parameters():
    ...
    Label_drop_lines.grid(row=0, column=2, columnspan=1, padx=5, sticky="nsew")
    
    drop_lines = OptionMenu(frame_tau_line, select_line, *lines_names)
    drop_lines.grid(row=1, column=2, pady=5, padx=5, ipady=3, sticky="nsew")
    ...
    def top_CG_Gas_Qantification_table():
        ...
        def calculo_Evolution_gas(line, gas, A, t, A_N2=0): # A:Area bajo la curva :: tau: tempo entre coletas, min :: 
            ...
            Evolution_gas = ((Area-A0)/b)*(Vr-Vs+Vt+Vl+(t*f))
