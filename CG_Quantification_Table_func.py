...
def top_CG_Gas_Qantification():
    ...
    length_of_lines = {'2':73, '4':272, '6':121, '8':314, '8L':298, '8S': 298+90.5, '8P':298+162.5, '12':724+260} # 8H change to 8L
    ...
    dic_lineas = {'2':Vt_value('2'), '4':Vt_value('4'), '6':Vt_value('6'), '8':Vt_value('8'), '8L':Vt_value('8L'), '8S':Vt_value('8S'), '8P':Vt_value('8P'), '8D':Vt_value('8L')+Vcopo+Vtubo1+Vtubo2, '12':Vt_value('12')}
    
    #print(dic_lineas)
    dic_Vol_reactor = {'2':100.9, '4':100.9, '6':100.9, '8':100.9, '8L':100.9, '8S':4, '8P':1, '8D':100.9, '12':75}# Volume do reator, ml 

    gases = ['CO', 'CH4', 'C2H6', 'O2', 'H2', 'CO2', 'N2']
    #parametros do H2 atualizados em 26-03-2025: de [0,3198] para [0,1127.055]
    #parametros do O2 atualizados em 26-03-2025: de [0,254] para [0,30.65486]
    
    parametros_ab = [[0,59292],[0,55129],[0,132900],[0,30.65486],[0,1127.055],[0,232.973], [1,1]]#For N2 a and b parameters does not have any meaning
    ...
    def calculo_Evolution_gas(line, gas, A, t, A_N2=0): # A:Area bajo la curva :: tau: tempo entre coletas, min :: 
        Area = A-(0.35*A_N2)#((0.21/0.78)*A_N2)
        ...
        # else:
        Evolution_gas = ((Area-A0)/b)*(Vr+Vt+Vl+(t*f)-Vs)
    ...
    def create_df_evolution():
        ...
        for gas in gases[:-1]: # disregards N2
            for i in range(len(df_in[gas])):
                ...
                if gas=='O2':
                    A_N2 = df_in.loc[i,'N2']
                    df_out.loc[i,gas] = calculo_Evolution_gas(Vt, gas, A, t, A_N2)
                else:
                    df_out.loc[i,gas] = calculo_Evolution_gas(Vt, gas, A, t)
        return df_out
            ...
            if gas=='CO' or gas=='CH4': #or ...
                for i in range(len(df_in[gas])):
                    ...
                    if i==0:
                        t = 0
                        Vt, gas, A, t = get_parameters(df_col, df_val, t) 
                        gas0 = calculo_Evolution_gas(Vt, gas, A, t)
                    else: 
                        t=int(tau_box.get())
                    Vt, gas, A, t = get_parameters(df_col, df_val, t) 
                    df_out.loc[i,gas] = calculo_Evolution_gas(Vt, gas, A, t)-gas0
            else:
                for i in range(len(df_in[gas])):
                    df_col = gas
                    df_val = df_in.loc[i,gas]
