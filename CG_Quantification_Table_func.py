...
def top_CG_Gas_Qantification_parameters():
    ...
    Label_carrier_gas.grid(row=0, column=4, columnspan=1, padx=5, sticky="nsew")
    var_carrier_gas = BooleanVar()
    var_carrier_gas.set(False)
    check_carrier_gas = Checkbutton(frame_tau_line, text="", width=5, variable = var_carrier_gas)
    check_carrier_gas.grid(row=1, column=4, pady=5, columnspan=1)
    
    test_button = Button(frame_tau_line, width=18, text="Click to Create Table ",
                      font=("", 10), background="#E8E8E8",
                      command=lambda: top_CG_Gas_Qantification_table())
    test_button.grid(row=0, rowspan=2, column=5, padx=10, pady=5, sticky="nsew")
    ...
    def top_CG_Gas_Qantification_table():
        ...
        Vmangeira = math.pi*247*(0.3/2)**2
        #print(Vcopo , '  :::  ', Vtubo1, '  :::  ', Vtubo2)
        
        length_of_lines = {'2':73, '4':272, '6':121, '8L':298, '8S': 298+90.5, '8P':298+162.5, '12':751} # 8H change to 8L
        
        dic_lineas = {'2':Vt_value('2'), '4':Vt_value('4'), '6':Vt_value('6'), '8L':Vt_value('8L'), '8S':Vt_value('8S'), '8P':Vt_value('8P'), '8D':Vt_value('8L')+Vcopo+Vtubo1+Vtubo2, '12':Vt_value('12')+Vmangeira}
        
        dic_Vol_reactor = {'2':100.9, '4':100.9, '6':100.9, '8L':100.9, '8S':4, '8P':1, '8D':100.9, '12':13.1892}# Volume do reator, ml 
    
        dic_Vol_sol = {'2':30, '4':30, '6':30, '8L':30, '8S':0, '8P':0, '8D':30, '12':0}# Volume da solução , ml 
    
        gases = ['O2', 'N2', 'H2', 'CH4', 'CO']
        ...
        ### Calibration Parameters updated on 05-09-2025
        # Own method
        # O2 --> [-0.0399, 27.6425]
        # N2 --> [0.1754, 84.5904]
        # H2 --> [-1.9249, 1000.4319]
        ...
        # O2 --> [-0.6251, 42.9118]
        # N2 --> [-2.0881, 134.8015]
        # H2 --> [-23.1869, 1664.8856]
        # CH4 --> [-27.5860, 528.3941]
        # CO --> [-38.5973, 737.5264]
        ###
        # # first parameter, as for example [1,2] will be used as [0,2] because discrepant values on the zero 
      
        ###############################
        def dic_param_ab():
            var = var_carrier_gas.get()
            line = select_line.get()

            if line == '2' or line == '4': # Only for specific method for O2, N2, H2 
                parametros_ab = [[0, 27.6425], [0, 84.5904], [0, 1000.4319], [0,1], [0,1]]
                dic_parametros_ab = dict(map(lambda i,j : (i,j) , gases,parametros_ab))
                var_carrier_gas.set(False)
                
            elif '8' in line: # Only for specific method for CO2 
                parametros_ab = [[0, 42.9118], [0, 134.8015], [0, 1664.8856], [0, 528.3941], [0, 737.5264]]
                dic_parametros_ab = dict(map(lambda i,j : (i,j) , gases,parametros_ab))
                var_carrier_gas.set(True)

            else: 
                if var == False: # Only for specific method for O2, N2, H2
                    parametros_ab = [[0, 27.6425], [0, 84.5904], [0, 1000.4319], [0,1], [0,1]]
                    dic_parametros_ab = dict(map(lambda i,j : (i,j) , gases,parametros_ab))
                    
                else: # Only for specific method for CO2
                    parametros_ab =  [[0, 42.9118], [0, 134.8015], [0, 1664.8856], [0, 528.3941], [0, 737.5264]]
                    dic_parametros_ab = dict(map(lambda i,j : (i,j) , gases,parametros_ab))
            
            return dic_parametros_ab
        ###############################   
        ...
        def get_parameters(df_col, df_val, t):
            ...
            dic_param = dic_param_ab()

            return line, gas, A, t, flux, dic_param
        
        def factor_O2_N2():
            dic_parametros_ab = dic_param_ab()
        ...
        def calculo_Evolution_gas(line, gas, A, t, flux, dic_param, A_N2=0): # A:Area bajo la curva :: tau: tempo entre coletas, min :: 
            dic_parametros_ab = dic_param
            ...
            for gas in gases[:]:
                for i in range(len(df_in[gas])):
                    ...
                    if i==0:
                        ...
                        t=int(tau_box.get())
                    Vt, gas, A, t, flux, dic_param = get_parameters(df_col, df_val, t) 
                    if gas=='O2':
                        A_N2 = df_in.loc[i,'N2']
                        df_out.loc[i,gas] = calculo_Evolution_gas(Vt, gas, A, t, flux, dic_param, A_N2)
                    else:
                        df_out.loc[i,gas] = calculo_Evolution_gas(Vt, gas, A, t, flux, dic_param)
            return df_out
        
        # def create_df_evolution_CO2_carrier():# Consider detectable inpurities on the CO2
        #     df_in, x = data_list(size)
        #     df_out = pd.DataFrame(columns=['time(min)', *gases], index=range(Ty))
        #     df_out['time(min)']=df_in['time(min)']
        ...
        def print_text():
            dic_parametros_ab = dic_param_ab()
            print("\nIs the carrier gas CO2?: ", var_carrier_gas.get()) 
            print('Calibration parameters being used: ', dic_parametros_ab)
        ...
        results_button = Button(frame_output, width=28, text="Click to See Results",
                          font=("", 10), background="#E8E8E8",
                          command=lambda: print_text())
        results_button.grid(row=0, column=0, padx=10, pady=5,  sticky="nsew")
    
        plot_button = Button(frame_output, width=28, text="Click to plot the Results",
                          font=("", 10), background="#E8E8E8",
        ...
        def top_output_Tool():
            ...
            def print_data_output():
                ...
                text_in = (str(datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
                            ...
                            + 'Is the carrier gas CO2? --> ' + str(var_carrier_gas.get()) +'\n\n'
