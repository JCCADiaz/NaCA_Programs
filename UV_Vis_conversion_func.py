...
def top_Ref_conv():
    ...
    def from_Ref_convert(path):
        ...
        ##########################################################
        data = pd.read_csv(path, sep="\t", decimal=",", skiprows = 43, encoding = 'ISO-8859-1', names = ['Wavelength(nm)','Reflectance'],
        ...
        # F(R)= (1-R)^2/(2*R) 
        if n==2:
            data['(F(R) h nu)^2(eV m-1)^2'] = ((((1-data['Reflectance']/100)**2)/(2*data['Reflectance']/100))*(h*c/(data['Wavelength(nm)']*1E-9)))**n
            data_out = data[['Wavelength(nm)', 'Reflectance', 'hc/lambda(eV)', '(F(R) h nu)^2(eV m-1)^2']].copy()   
        if n==1/2:
            data['(F(R) h nu)^1/2(eV m-1)^1/2'] = ((((1-data['Reflectance']/100)**2)/(2*data['Reflectance']/100))*(h*c/(data['Wavelength(nm)']*1E-9)))**n
...
def top_Tra_conv():
    top_Tra = Toplevel()
    top_Tra.title("Transmitance_Tool")
    top_Tra.geometry("360x365+330+450")
    ...
    def from_Tra_convert(path):
        ...
        thickness = float(n_box.get())*1E-3# mm
        get_n = n_box.get()
        if get_n=='1/2':
            get_n = 0.5 
        n = float(get_n) # 2 Direct, 1/2 Indirect
        ##########################################################
        df_T_A = pd.read_csv(path, sep="\t", decimal=",", skiprows = 43, encoding = 'ISO-8859-1', names = ['Wavelength(nm)','Transmittance(%)'],
                dtype={'Wavelength(nm)': 'float64', 'Transmittance(%)': 'float64'})

        df_T_A['Absorbance'] = -1*np.log10(df_T_A['Transmittance(%)']/100)
        
        ###################################
        df_T_A['hc/lambda(eV)'] = h*c/(df_T_A['Wavelength(nm)']*1E-9)
        if n==2:
            df_T_A['(alpha h nu)^2(eV m-1)^2'] = ((df_T_A['Absorbance']*h*c)/(thickness*math.log10(math.e)*df_T_A['Wavelength(nm)']*1E-9))**n
        if n==1/2:
            df_T_A['(alpha h nu)^1/2(eV m-1)^1/2'] = ((df_T_A['Absorbance']*h*c)/(thickness*math.log10(math.e)*df_T_A['Wavelength(nm)']*1E-9))**n
        ###################################
        return df_T_A
    ...
    def view_Data():
        ...
        if isinstance(data, pd.DataFrame) == True:
            ...
            # Plot (alpha h nu)^n Vs. hc/lambda
            plot2.plot(data[columns[3]], data[columns[4]])
            ...
            for key in data:
                ...
                # Plot (alpha h nu)^n Vs. hc/lambda
                plot2.plot(data[key][columns[3]], data[key][columns[4]])
                
        plot1.set_xlabel('Wavelength (nm)')
        plot1.set_ylabel('Transmittance (%)')
        plot2.set_xlabel('hc/lambda (eV)')
        plot2.set_ylabel('(alpha h nu)^n (eV m-1)^n')
    ...
    ##############################
    # Between frame select file
    frame_select_file = LabelFrame(top_Tra, padx=5, pady=5)#padx and pady inside of the frame
    # in frame text="..." is optional
    ...
    ###############################
    # Between frame Abs quantity
    frame_Abs_quantity = LabelFrame(top_Tra, text="Quantity of files", padx=5, pady=5)#padx and pady inside of the frame
    # in frame text="..." is optional
    frame_Abs_quantity.grid(row=0, column=0, padx=10, pady=5, sticky="nsew")#padx and pady outside of the frame
    ...
    ###############################
    # Between frame UVVis
    frame_file_UVVis = LabelFrame(top_Tra, text="", padx=5, pady=5)#padx and pady inside of the frame
    ...
    t_label= Label(frame_file_UVVis, font=("", 8), text="thickness of the sample (mm)", width=23)
    t_label.grid(row=2, column=0, pady=5)
    
    t_box = Entry(frame_file_UVVis, font=("", 12), width=17)
    t_box.insert(0, "1")
    t_box.grid(row=3, column=0, pady=5)
    ...
    view_file = LabelFrame(top_Tra, text="View Measurement Without Saving", padx=5, pady=5)#padx and pady inside of the frame
